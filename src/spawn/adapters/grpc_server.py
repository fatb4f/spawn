...
class SpawnControlService(pb2_grpc.SpawnControlServicer):
    ...
    def CodexSessionRefresh(
        ...
        log_path = (
            Path(request.log_path).expanduser()
            if request.log_path
            else codex_session_ops.default_log_path()
        )
        rc, out, err, rid = codex_session_ops.dispatch_refresh(
            request_id=request.request_id or None,
            event_id=request.event_id or None,
            refresh_command=request.refresh_command
            or "codex-refresh-context --wait-session-write",
            log_path=log_path,
        )
    ...
    def CodexSessionStatus(
        ...
        if not rid:
            return pb2.CodexSessionStatusResponse(
                ok=False, message="no request_id available"
            )
        row = codex_session_ops.status_row(rid, log_path, request.wait_seconds or 2.0)
        if row is None:
            return pb2.CodexSessionStatusResponse(
                ok=False, request_id=rid, message="request not found"
            )
        return pb2.CodexSessionStatusResponse(
            ok=True, request_id=rid, row_json=json.dumps(row, sort_keys=True)
        )

    def CodexSessionLogs(
        self, request: pb2.CodexSessionLogsRequest, context: grpc.ServicerContext
    ) -> pb2.CodexSessionLogsResponse:
        log_path = codex_session_ops.default_log_path()
        rid = codex_session_ops.resolve_request_id(request.request_id, log_path)
        if not rid:
            return pb2.CodexSessionLogsResponse(
                ok=False, message="no request_id available"
            )
        rc, out, err = codex_session_ops.read_journal_logs(rid, request.lines or 100)
        return pb2.CodexSessionLogsResponse(
            ok=rc == 0, request_id=rid, logs=out, return_code=rc, message=err
        )

    def CodexSessionList(
        self, request: pb2.CodexSessionListRequest, context: grpc.ServicerContext
    ) -> pb2.CodexSessionListResponse:
        log_path = (
            Path(request.log_path).expanduser()
            if request.log_path
            else codex_session_ops.default_log_path()
        ...
        return pb2.CodexSessionListResponse(
            ok=True, rows_json=json.dumps(rows, sort_keys=True)
        )
