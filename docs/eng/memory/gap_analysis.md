...
| G7 | P1 | No CI enforcement for generation/schema gates | resolved via `.github/workflows/ci.yml` | Drift reaches main branch undetected | Add CI for `scripts/check_gen.sh` + schema validation | done |
...
| G9 | P2 | No integration tests for CLI/daemon contracts | resolved via golden replay + CI e2e ingest/rebuild | regression risk for RPC and schema behavior | Add baseline integration + replay tests | done |
